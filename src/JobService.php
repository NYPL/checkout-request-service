...
class JobService
{
    const JOB_SUCCESS_MESSAGE = 'Job finished successfully for checkout request.';
    const JOB_FAILURE_MESSAGE = 'Job finished unsuccessfully for checkout request.';
     ...
     */
    public static function beginJob(CheckoutRequest $checkoutRequest, $message = '')
    {
        self::initializeJobClient();
        self::buildJobNotice((array)$checkoutRequest, $message);
        self::getJobStatus()->setNotice(self::getJobNotice());

        APILogger::addDebug(
            'Job is being initiated via the Job Service API. (ID: ' . $checkoutRequest->getId() . ')',
            [self::getJobStatus()]
        );

        self::getJobClient()->startJob(
            new Job(['id' => $checkoutRequest->getCheckoutJobId()]),
            self::getJobStatus()
        );
    }
     ...
     * @param CheckoutRequest $checkoutRequest
     */
    public static function finishJob(CheckoutRequest $checkoutRequest)
    {
        self::initializeJobClient();
        $data = (array)$checkoutRequest;

        try {
            self::buildJobNotice($data, self::JOB_SUCCESS_MESSAGE . ' (CheckoutID: ' . $checkoutRequest->getId() . ')');
            self::getJobStatusSuccess()->setNotice(self::getJobNotice());

            APILogger::addDebug(
                'Success status sent to the Job Service API. (CheckoutID: ' . $checkoutRequest->getId() . ')'
            );
            ...
            APILogger::addInfo(
                'Job threw an exception. ' . $exception->getMessage() . '. (CheckoutID: ' . $checkoutRequest->getId() . ')'
            );
        }
    }
